...
class _HomePageState extends State<HomePage> {
  ...
  void _toggleExercise(Exercise ex) {
    ...
    if (_exerciseTimers[ex.name] != null) {
      ...
      activityHistory.add(
        "${ex.name} stopped ⏹ at ${TimeOfDay.now().format(context)}",
      );
    } else {
      starting = true;
      _exerciseTimers[ex.name] = Timer.periodic(const Duration(seconds: 1), (
        timer,
      ) {
        setState(() {
          ex.time++;
      ...
      activityHistory.add(
        "${ex.name} started 🏁 at ${TimeOfDay.now().format(context)}",
      );
    }
  }
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  _buildActivityButton(
                    ...
                    totalSteps.toString(),
                    _isWalking,
                    const Color.fromARGB(255, 74, 8, 187),
                  ),
                  const SizedBox(width: 8),
                  _buildActivityButton(
                    "Swimming",
                    _formatTime(swimmingTime),
                    _isSwimming,
                    const Color.fromARGB(255, 228, 3, 228),
                  ),
                ],
              ),
              ...
              GridView.count(
                ...
                children: [
                  ...
                  _buildStatCard(
                    "Running",
                    "${runningKm.toStringAsFixed(2)} km",
                    _formatTime(runningTime),
                    colors[2],
                    Icons.directions_run,
                  ),
                  _buildStatCard(
                    "Walking",
                    _formatTime(walkingTime),
                    "${walkingCalories.toStringAsFixed(1)} kcal",
                    colors[3],
                    Icons.timer,
                  ),
                  _buildStatCard(
                    "Swimming",
                    _formatTime(swimmingTime),
                    "${swimmingCalories.toStringAsFixed(1)} kcal",
                    colors[5],
                    Icons.pool,
                  ),
                ],
                ...
                Expanded(
                  child: TextField(
                    ...
                    decoration: const InputDecoration(
                      ...
                      filled: true,
                      fillColor: Color.fromARGB(255, 100, 163, 159),
                    ),
                  ),
                ),
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Column(
          ...
          children: [
            Text(
              "Time: ${_formatTime(ex.time)}",
              style: const TextStyle(color: Color.fromARGB(255, 128, 10, 240)),
            ),
            const SizedBox(height: 4),
            LinearProgressIndicator(
              value: progress,
              color: const Color.fromARGB(255, 253, 205, 255),
        ...
        trailing: ElevatedButton(
          ...
          style: ElevatedButton.styleFrom(
            backgroundColor: isActive
                ? const Color.fromARGB(255, 76, 221, 107)
                : Colors.white,
            shape: RoundedRectangleBorder(
